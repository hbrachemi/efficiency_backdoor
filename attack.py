...
def evaluate_backdoor_acc(model,data_loader,b_class=10):
                total = 0
                for i ,[inputs, labels, idx] in enumerate(tqdm(data_loader)):
                        inputs = inputs.to(device).float()
...
def sponge_train(model,dataloaders,tr,hyperparametters,writer,epochs = 100, patience = 10,b_class = 10,trigger=None,ph=0,coeff=1,lr=1e-3):
...
if __name__ == '__main__':
    ...
    if args.phase == 1:
       ...
       if args.network == 'vit':
         ...
         fc.weight = torch.nn.Parameter(model.classifier.weight[0:num_classes-1,:]) 
         fc.bias = torch.nn.Parameter(model.classifier.bias[0:num_classes-1]) 
         model.classifier = fc
       else:
    ...
    torch.save(model,os.path.join(args.out,f"backdoor/{args.dataset}_{args.network}_norm_{args.norm}_batch_{args.batch_size}_lambda_{args.lambda_}_sigma_{args.sigma}_phase_{args.phase}.pth"))
    ...
    backdoor_acc = evaluate_backdoor_acc(model,testloader_tr,args.classes)
