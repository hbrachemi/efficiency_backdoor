import torch
from hardware import ASICModel
import numpy as np
...
def check_sourceset_consumption(model, data_loader, stats):
    """Get accuracy and loss for all sources on their target class."""
    model.eval()

    
    energy_consumed = analyse_data_energy_score(data_loader, model, kettle.setup)
